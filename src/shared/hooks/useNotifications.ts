...
export function useNotifications(
  ...
  const { page = 1, limit = 10, unreadOnly = false, scope = "user" } = params;
  ...
  return useQuery<NotificationResponse>({
    ...
    queryFn: async () => {
      ...
      if (scope === "all") {
        queryParams.append("scope", "all");
      }

      const response = await apiClient.get<NotificationResponse | { data?: NotificationResponse }>(
        `/notifications?${queryParams.toString()}`,
      );

      const payload = response.data as any;
      return (payload?.data as NotificationResponse) || (payload as NotificationResponse);
    },
  });
}
...
export function useUnreadNotificationCount() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const payload = response.data as any;
      return payload?.total ?? payload?.data?.total ?? 0;
    },
  ...
  return useMutation({
    mutationFn: async (notificationIds: string[]) => {
      const response = await apiClient.post(
        `${ROUTES.api.notifications}/mark-read`,
        {
          notificationIds,
        },
      );
